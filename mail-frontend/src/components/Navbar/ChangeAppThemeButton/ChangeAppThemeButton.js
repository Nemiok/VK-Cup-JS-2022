...
import { useTheme } from '../../../customHooks/hooks';
import './ChangeAppThemeButton.css';

const ChangeAppThemeButton = () => {
  const { theme, setTheme } = useTheme();

  const handleOnDarkThemeSelect = () => {
    setTheme('dark');
  };

  const handleOnLightThemeSelect = () => {
    setTheme('light');
  };

  const handleChangeThemeButtonClick = () => {
    theme === 'dark' ? handleOnLightThemeSelect() : handleOnDarkThemeSelect();
  };


  return (
    <button onClick={handleChangeThemeButtonClick} tabIndex={0} className='navbar__change-app-theme-button'>
